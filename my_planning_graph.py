...
class PlanningGraph():
    ...
    def add_action_level(self, level):
        ...
        in state nodes. The socond would require to check if prenodes are in state, which is
        easier
        """
    ...
    def add_literal_level(self, level):
        ...
        # iterating over actions of previous action level
        for action in self.a_levels[level - 1]:
            # every effect of actions must be added to the next state level
        ...
        for s_node in self.s_levels[level]:
            ...
            for action in self.a_levels[level - 1]:
                # iterating over effects of those actions
                for effect in action.effnodes:
    ...
    def inconsistent_effects_mutex(self, node_a1: PgNode_a, node_a2: PgNode_a) -> bool:
        ...
        for pos_effect in node_a1.action.effect_add:
            # assuming we can compare using in
    ...
    def interference_mutex(self, node_a1: PgNode_a, node_a2: PgNode_a) -> bool:
        ...
        # checking if positive effect of action 1 is a negative precondition for a2
        for pos_effect in node_a1.action.effect_add:
        ...
        # checking is a negative effect of a1 is in positive preconditions of a2
    ...
    def negation_mutex(self, node_s1: PgNode_s, node_s2: PgNode_s) -> bool:
        ...
        if node_s1.symbol == node_s2.symbol:
            if node_s1.is_pos != node_s2.symbol:
                return True
        return False
    ...
    def inconsistent_support_mutex(self, node_s1: PgNode_s, node_s2: PgNode_s):
        ...
        :return: bool
        """
        # checks if parent actions for s1 and s2 are all mutex.
        mutex = False
        for parent_action_s1 in node_s1.parents:
            for parent_action_s2 in node_s2.parents:
                if parent_action_s1 == parent_action_s2:
                    return False
    ...
    def h_levelsum(self) -> int:
        ...
        """
        level_sum = 0
        goals_to_find = self.problem.goal[:]
        for i, state_level in enumerate(self.s_levels):
            for s_node in state_level:
                for goal in goals_to_find:
                    if s_node.symbol == goal and s_node.is_pos:
                        level_sum += i
                        goals_to_find.remove(goal)
